...
shared_ptr<Sprite_old> Sprite_old::CreateFromWIL(ID3D11Device* device, ImageLib::ImageInfo& info, std::vector<uint32_t>&& rgba32)
{
	if (rgba32.size() <= 0)
		return {};

	auto sprite = std::make_shared<Sprite_old>();
...
shared_ptr<Sprite_old> Sprite_old::CreateRenderTarget(ID3D11Device* device, int16_t w, int16_t h, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	auto sprite = std::make_shared<Sprite_old>();
